...
> **Note:** sections marked as *full mode only* will not appear when running with `--minimal` or when the dataset exceeds 5 million cells. This is expected — those analyses are skipped intentionally to keep runtimes manageable on large files.
